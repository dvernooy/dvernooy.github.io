...
** main.c
** 7-20-13
** version 6 ported from version 5, version 6 currently on chip 31874 bytes
** 7-20-13: EEPROM read updated, compiled, tested, & works w/ book6.xlsm on laptop & car
** 7-20-13: PIPE2XL updated, compiled & runs on car but output is screwy
** 7-20-13: PING updated, compiled ... seems to hang on init on car
** VBA code in BOOK6.xls, 3 tabs, tweaked VBA code to use USB/serial
** based on ATMEGA 88 testing ... code included in folders
*/
...
void clear_codes (void) {
 ...
 while(1) {
	...
	fprintf_P(&lcd_out,PSTR("%-7.0f"), (double)(temp_get[5]-40));
	lcd_goto_xy(6,4);
	fprintf_P(&lcd_out,PSTR("%-7.0f"), (double)((IAT_get[5]-40)+273.0));
	} //end if switch
 } //end while
...
if (found > 0){
  ...
  lcd_goto_xy(1,2);
  fprintf_P(&lcd_out, PSTR("MEM: %d/17"), found);
    }
 if (found ==0) {
  lcd_clear(); 
  lcd_goto_xy(1,1);
